...
for spp in species_samples:
	if len(species_samples[spp])>1:
		...
		for pair_A in itertools.combinations(species_samples[spp],2):
			print spp,spp,pair_A,dist_mtx[pair_A[0]][sample_name_list.index(pair_A[1])]
			dist = int(dist_mtx[pair_A[0]][sample_name_list.index(pair_A[1])])
			dist_A.append(dist)
		for pair_B in itertools.product(species_samples[spp],rest_individual_list):
			dist = int(dist_mtx[pair_B[0]][sample_name_list.index(pair_B[1])])
			dist_B.append(dist)
			print spp,name_dict[pair_B[1]],pair_B,dist_mtx[pair_B[0]][sample_name_list.index(pair_B[1])]
		print '#',sorted(dist_A),sorted(dist_B)
